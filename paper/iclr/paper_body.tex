...
\textbf{Cost.} 5,420 hosted calls for \$2.99 in metered spend, plus about two hours of local GPU time for cloning and the open-weight model. Total spend is in Appendix~\ref{app:cost}.
...
The RESYNTH arm is the same real waveform after a 6 kbps neural-codec round trip. Flash grades it 8.69, which is $-0.312$ points against real (interval $-0.460$ to $-0.162$, $p = 3.5\times10^{-4}$, $d_z = -0.25$, $n = 200$) and $-0.320$ against the clone ($p = 4.5\times10^{-4}$). Both differences survive correction; they are the only two of the 45 tests that do. Word error rate on RESYNTH is 3.1\%, not different from real ($p = 0.86$), and comprehension is 99\%. So the codec lowers the grade without lowering intelligibility.
...
D1 (implicit exceeds explicit): not supported on either Gemini model. D2 (resynthesis explains under half of the clone effect): reversed; the clone effect is zero and the resynthesis effect is the only one present. D3 (sign differs by family, Gemini at or above real): the Gemini half holds, with clone at or slightly above real on both tiers; @@VOXD3@@ D4 (explicit detection under 60\%): holds, at 0.555 and 0.54. D5 (word error rate equal): holds, 2.7\% against 2.9\%, $p = 0.87$.
...
Comprehension is at ceiling on every arm, so O3 cannot show a difference. It serves only as an intelligibility check.
...
Gemini 3.1 Pro ran 60 items, not 200, because of cost; its intervals are three times wider. @@VOXLIMIT@@ Two hosted audio families we intended to test were unavailable to us at the time of the study.
